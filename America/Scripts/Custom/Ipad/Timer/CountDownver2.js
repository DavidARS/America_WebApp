...
function sendUserSession(restart) {
	//
	//
	//
 	// edits 07.01.20 das - block the call to the vadar lab server (it collected user data)
    /*
    $.ajax({
        type: 'GET',
    ...
    */
     // end edits 07.01.20 das
    //
    // edits 07.01.20 das - need to add option for the commented out vadar block
    //
	function restartAndTimer() {
        sendMessage('sendUserSession success!');
        console.log('success: ');
		if (restart) {
			window.location.reload();
		}
...
$('#Finish_button').click(function(e){
    ...
    if($(this).val() != "Next Challenge"){
        ...
        $('#Restart_button').attr('disabled',true);

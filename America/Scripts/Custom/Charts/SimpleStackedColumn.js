...
function CreateHighchartSimple($jsonObj,controlID,subcontrols, fldLabName, fldValue, Name, index) {
    ...
    if (Name == 'Resources') {
        ResourceNames[0] = "Surface Water (Fresh)";
        ResourceNames[1] = "Lake";
        ResourceNames[2] = "Surface Water (Saline)";
        ResourceNames[3] = "Ground Water";
        ResourceNames[4] = "Reclaimed";

    }
    else {
        ResourceNames[0] = "Community Water";
        ResourceNames[1] = "Agriculture";
        ResourceNames[2] = "Industry";
        ResourceNames[3] = "Power Production ";
    }
        // Get Data
        DData = new Array();
        NDData = new Array();
         ...
         // ==================
            ...
            $.each(flds, function () {
                ...
                if (fldValue[this] != undefined) {
                    ...
                    if (Name == 'Resources') {
                        legendText = 'Supply';
                        if (fld == 'SUR') { DData[0] = fldValue[this][index] }
                        if (fld == 'SURL') { DData[1] = fldValue[this][index] }
                        if (fld == 'SAL') { DData[2] = fldValue[this][index] }
                        if (fld == 'GW') { DData[3] = fldValue[this][index] }
                        if (fld == 'REC') { DData[4] = fldValue[this][index] }
                    }
                    else if (Name == 'Consumers') {
                        legendText = 'Demand Met';
                        if (fld == 'UD') { DData[0] = fldValue[this][index] }
                        if (fld == 'AD') { DData[1] = fldValue[this][index] }
                        if (fld == 'ID') { DData[2] = fldValue[this][index] }
                        if (fld == 'PD') { DData[3] = fldValue[this][index] }
                        //
                        if (fld == 'UDN') { NDData[0] = fldValue[this][index] }
                        if (fld == 'ADN') { NDData[1] = fldValue[this][index] }
                        if (fld == 'IDN') { NDData[2] = fldValue[this][index] }
                        if (fld == 'PDN') { NDData[3] = fldValue[this][index] }
                    }
                }});

...
function CreateHighchart($jsonObj, controlID, subcontrols, AFluxList) {
    ...
    for (ri = 0; ri < ResFields.length; ri++) {
        ...
        ConsFluxList = ResFluxList.GetConsumerFluxes("UD");
        Allocated = ConsFluxList.List[0].LastValue();
        UData[ri] = Allocated;
        // get the AD list, should only be one
        ConsFluxList = ResFluxList.GetConsumerFluxes("AD");
        Allocated = ConsFluxList.List[0].LastValue();
        AData[ri] = Allocated;
        // get the ID list, should only be one
        ConsFluxList = ResFluxList.GetConsumerFluxes("ID");
        Allocated = ConsFluxList.List[0].LastValue();
        IData[ri] = Allocated;
        // get the PD list, should only be one
        ConsFluxList = ResFluxList.GetConsumerFluxes("PD");
        Allocated = ConsFluxList.List[0].LastValue();
        PData[ri] = Allocated;
    }
